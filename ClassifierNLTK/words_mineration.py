...
# ## Applying stopwornds in the model
...
# Excluding stopwords
def exclud_stopword(texto):
    frases=[]
    for (palavras,emocao) in texto: 
        semstop=[p for p in palavras.split() if p not in stopwordsnltk]
        frases.append((semstop,emocao))
    return frases
print(exclud_stopword(base))
...
palavras=buscapalavras(basetreinamento)
#rint(palavras)


# # Listing over words from database
...
# # Extracting words from any senteces
...
# ## Extracting words from all sentences
...
# Apresentadando as palavras mais vitais para os classificadores
...
#esperado='alegria alegria alegria alegria medo medo surpresa'.split()
#previsto='alegria alegria medo surpresa medo medo surpresa'.split()
...
with open('classificador_palavras.pkl', 'wb') as f:
